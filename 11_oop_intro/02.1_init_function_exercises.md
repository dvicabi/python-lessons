...
1. צרו מחלקה בשם `Book` עם פעולת `__init__` שמקבלת `title` ו־`author`, ושומרת אותם באובייקט.

2. הוסיפו למחלקה `Book` פעולה בשם `describe()` שמדפיסה:  
...
5. הוסיפו למחלקה `Rectangle` פעולה בשם `area()` שמחזירה את השטח (רוחב כפול גובה).
